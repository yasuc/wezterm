...
local ICONS = {
  ...
  opencode = wezterm.nerdfonts.md_robot_outline,
...
-- 現在実行中のOpen Codeセッションをスキャン
local function scan_active_opencode_sessions()
  local sessions = {}
  ...
  for _, mux_window in ipairs(wezterm.mux.all_windows()) do
    ...
    for _, tab in ipairs(mux_window:tabs()) do
      ...
      for _, pane_info in ipairs(tab:panes_with_info()) do
        ...
        if process_name and process_name:find("opencode") then
...
local function create_fzf_session_selector()
  return wezterm.action_callback(function(window, pane)
    local sessions = scan_active_opencode_sessions()

    if not sessions or #sessions == 0 then
      window:toast_notification("Active Open Code Sessions", "No active Open Code sessions found", nil, 4000)
      return
    end

    local temp_dir = os.getenv("TMPDIR") or "/tmp"
    local sessions_file = temp_dir .. "/wezterm_opencode_sessions_" .. os.time() .. ".jsonl"
    local formatted_file = temp_dir .. "/wezterm_opencode_fzf_input_" .. os.time() .. ".txt"
    local result_file = temp_dir .. "/wezterm_opencode_result_" .. os.time() .. ".txt"
    local port_file = temp_dir .. "/wezterm_opencode_port_" .. os.time() .. ".txt"

    if not export_sessions_to_file(sessions, sessions_file) then
      window:toast_notification("Active Open Code Sessions", "Failed to export session data", nil, 4000)
      return
    end

    if not export_formatted_sessions_to_file(sessions, formatted_file) then
      window:toast_notification("Active Open Code Sessions", "Failed to format session data", nil, 4000)
      os.remove(sessions_file)
      return
    end

    local config_dir = wezterm.config_dir or (os.getenv("HOME") .. "/.config/wezterm")
    local preview_script = config_dir .. "/scripts/preview_opencode_session.lua"
      ...
      local function check_pane_closed()
        ...
        if pane_exists then
          ...
          if refresh_counter >= 5 then
            ...
            sessions = scan_active_opencode_sessions()
...
function module.apply_to_config(config)
  wezterm.log_info("opencode_session module loaded")

  -- LEADER+o: 現在実行中のOpen Codeセッションを検索して切り替え
  table.insert(config.keys, {
    key = "o",
    mods = "LEADER",
    action = create_fzf_session_selector(),
  })
  wezterm.log_info("opencode_session keybinding registered: LEADER+o (active sessions with fzf)")
end
