...
local function is_opencode_process(process_name, full_path)
	return process_name == "opencode" or (full_path and full_path:find("opencode"))
end
...
local function get_icon_and_color(process_name, pane_title, cmdline, cwd, is_ssh, is_active, is_opencode)
	if is_ssh then
	...
	if is_opencode then
		return ICONS.opencode, ICON_COLORS.opencode
	end
...
function module.apply_to_config(config)
	...
	wezterm.on("update-status", function(_, pane)
		...
		local full_path = pane:get_foreground_process_name() or ""
		if is_opencode_process(process_name, full_path) then
			opencode_cache[pane_id] = true
		elseif process_name ~= "zsh" and process_name ~= "bash" and process_name ~= "fish" then
			opencode_cache[pane_id] = nil
		end
	end)
	...
	wezterm.on("format-tab-title", function(tab, _, _, _, _, max_width)
		...
		-- Open Code検出（キャッシュ + format-tab-titleから直接検出）
		local is_opencode = opencode_cache[pane_id] or is_opencode_process(process_name, cmdline)
		...
		local icon, icon_color =
			get_icon_and_color(process_name, pane_title, cmdline, cached_cwd, is_ssh, tab.is_active, is_opencode)
